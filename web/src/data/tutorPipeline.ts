...
export const TUTOR_PIPELINE: { nodes: PipelineNode[]; edges: PipelineEdge[] } = {
  nodes: [
    ...
    {
      id: "wiki",
      label: "Wikipedia augment",
      desc: "Fetches one Wikipedia summary per concept (always-on; TUTOR_DEEP_WIKI=0 disables) and appends them AFTER the corpus sources at trailing ranks — corpus stays primary, wiki is supplementary. Pure code, no LLM; silent-degrade on network failure; skipped entirely when the corpus returned nothing.",
      kind: "data",
      stage: null,
      locked: true,
      defaultModel: "Wikipedia REST",
    },
    {
      id: "def_recovery",
      label: "Definition recovery",
      desc: "Dedicated retrieval/extraction of verbatim formal definitions (definition, theorem, lemma, etc.) from source texts. Runs a targeted hybrid search per detected definitional gap, scores token-recall against source chunks, and formats recovered statements as labelled blockquotes. Pure code; best-effort — never blocks the answer on failure.",
      kind: "data",
      stage: null,
      locked: true,
      defaultModel: "targeted hybrid search → token-recall",
    },
    {
      id: "image_judge",
  ...
  edges: [
    ...
    { from: "coverage",   to: "wiki" },
    { from: "wiki",       to: "def_recovery" },
