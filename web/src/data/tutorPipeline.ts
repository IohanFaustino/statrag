...
export const TUTOR_PIPELINE: { nodes: PipelineNode[]; edges: PipelineEdge[] } = {
  nodes: [
    ...
    },
    {
      ...
      desc: "Gap-triggered, best-effort. Detects concepts whose defining equation was OCR-dropped to an image, then per gap in parallel: global formula cache → vision reads the equation off the figure (gpt-4o) → text re-query fallback. Recovered LaTeX is injected into the synth as a <recovered_equations> block (used verbatim) and cached. Degrades to no-op on any failure.",
      kind: "data",
      stage: null,
    ...
    },
    {
      ...
      desc: "Writes the structured multi-aspect answer. The main model. Uses any recovered equations verbatim.",
      kind: "llm",
      stage: "draft",
  ...
  edges: [
    ...
    { from: "drafting", to: "formula_recovery" },
    { from: "formula_recovery", to: "draft" },
