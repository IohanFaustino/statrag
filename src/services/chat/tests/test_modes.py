...
def test_registry_has_4() -> None:
    """ModeRegistry must contain exactly 4 entries after registration."""
    from src.services.chat.modes import ModeRegistry, register_all_modes

    register_all_modes()
    ids = {m.id for m in ModeRegistry.all()}
    assert ids == {"tutor", "qa", "facilitate", "resume"}
...
def test_register_all_modes_is_idempotent() -> None:
    ...
    assert count_after_double_call == 4
