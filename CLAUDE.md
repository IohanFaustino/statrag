...
| **Extension mode — finish/polish + restore old chats** — new `extension` mode (deepagents topology C; corpus+Wikipedia footnote augmentation; styled-HTML ZIP export) is **BUILT + live-proven end-to-end** on branch `worktree-feat+extension-mode` (770+ backend / 207 frontend tests green; 7 live/review fixes committed; real Wikipedia footnotes w/ LaTeX rendered). The extension mode + its 8 conversations live ONLY in THIS worktree: code (`src/services/chat/extension_agents/`, `extension_skills/`) + its OWN `data/chat.db` (8 `extension` convs incl. "Extend Hansen ch7 7.4–7.5", conv `9d9985d3…`). **Run the recovered last version with `./scripts/dev.sh` FROM this worktree** (Qdrant shared on :6333). **Open follow-ups:** (1) **OLD-CHAT / ANSWER RECOVERY (root cause found 2026-06-09)** — chats "vanished" only because each git worktree has its OWN `data/chat.db`; launching `dev.sh` from a different checkout (e.g. `feat/component-equation-enforcement`) reads a different DB. NOT a path bug — run from the worktree to see them. SEPARATE issue: the early extension runs persisted ONLY the user question, never the assistant answer (pre-`c8f52a3` GeneratorExit-on-disconnect skipped post-loop persist). The answer CONTENT survives only as on-disk artifacts in this worktree — `curated/timeline.md` (curated prose), `footnotes/*.md` + `footnotes/_final_output.md` (KaTeX footnotes), `conversation_history/*.md` (agent scratch logs). No `ExtensionDigest` JSON was ever serialized. **In progress:** backfill `9d9985d3…` assistant message from those artifacts (DB backed up to `data/chat.db.bak-*`). `c8f52a3` ("persist assistant turn in finally guard") IS in this worktree, so NEW runs persist correctly. (2) **Verify frontend→backend routes** (all `/api/*` incl. `/api/export` download). (3) live-confirm populated digest persists+re-renders after reload (persist-in-`finally` fix committed, untested live). (4) throughput: parallelize analysts + FS section-offload (full-chapter top-model ≈6min + 429 TPM). (5) math delimiters `\(…\)`→`$`/KaTeX + language drift (Polish leaked) + markdown `[^n]` markers. (6) subtopic scope-narrowing inconsistent. (7) browser visual verify (card + (i) modal + ZIP click). (8) rag-verify + merge into `feat/component-equation-enforcement` (NOT main). | ⏳ **NEXT — flagged 2026-06-09, built; answer-recovery + polish + merge pending.** | [spec](docs/superpowers/specs/2026-06-09-extension-mode-design.md) · [plan](docs/superpowers/plans/2026-06-09-extension-mode.md) · [doc 54](docs/services/chat-features/54-extension-mode.md) |

> **Session note (2026-06-09) — sibling-branch fix not yet in this worktree:** on `feat/component-equation-enforcement` a black-screen bug was fixed where opening an old **Q&A** conversation crashed the whole app — `QAAnswerCard` did an unguarded `scope.target_gap` on legacy/deepagent payloads that lack `scope`, and there was no error boundary so the React tree unmounted. Fix = guard `scope?` + support `thesis/deepening/synthesis` shape + add `StructuredErrorBoundary` around the structured render. **This worktree's `QAAnswerCard` still has the latent bug** (harmless here only because this DB holds no QA convs). Port the guard + error boundary before merge.

> **⏳ RESUME HERE (2026-06-09 late, live-verify of quality rebuild) — Extension mode end-to-end live run + 4 fixes, ONE step left:** ran the real prompt *"Extend section 7.4 (Chebyshev) and 7.5 (WLLN) of Hansen Probability and Statistics for Economists"* on :5175 from this worktree and monitored front+back+LLM. **First run died** with `GraphRecursionError` ("cannot schedule new futures after shutdown") — the rebuilt pipeline exceeds langgraph's default `recursion_limit=25`. Fixed + committed (`2fc8af7`): (a) `_run_round` now passes `EXTENSION_RECURSION_LIMIT` (default **100**); (b) `run_extension` now **emits the `meta` event first** (badge was stuck "TUTOR MODE", mode/model unset); (c) frontend `done` reducer **preserves an existing `error` status** (backend emits `error`→`done`; errors were rendered invisibly). **Second run succeeded**: 1052s (~17½ min), `ExtensionDigest` persisted, **10 points × 2 footnotes each**, badge "EXTENSION MODE", **188 KaTeX spans** in footnotes. But found render gap → fixed + committed (`6c192a4`): point **titles + curated_text + unfilled_gaps** showed raw `$…$`/`\(…\)` (only footnote bodies were KaTeX'd). Renamed `renderFootnoteBody`→`renderMathText`, applied to all 4 fields; backend now normalizes math delimiters on titles + unfilled_gaps too. Tests green (22 card+state front, 16 runner back). **LEFT TO DO:** (1) ✅ **DONE 2026-06-10 (commit `8888dcb`)** — visual verify on :5175 found 3 render gaps on RELOAD of the persisted conv (`6c192a4`'s backend normalization only applies at generation time, persisted digest still had legacy delimiters): point titles leaked raw `\(…\)`, footnote bodies leaked lone-`$`-on-own-line display blocks, and badge fell back to "TUTOR MODE" (`VALID_MODES` lacked "extension"). Fixed in frontend: `renderMathText` now normalizes `\(…\)`/`\[…\]`/own-line-`$` → `$`/`$$` before splitting; `mapConversationMessages` accepts "extension". Live-verified conv `24338206` on :5175: badge EXTENSION MODE, 273 KaTeX spans, 59 display blocks, ZERO raw-delimiter leaks across titles/curated/footnotes/gaps; no console errors. (2) ✅ **DONE** — `npx tsc -p web --noEmit` exits 0 (thinking-casts → `as unknown as ChatEvent`, `globalThis.fetch`, `ev.label` narrowing in chat.ts stage case). Remaining cosmetic: raw `**…**` markdown bold visible in footnote bodies (renderMathText doesn't render markdown — part of open follow-up 5). (3) backend `digest.book` came back **"Unknown"** + chapter **"7.4–7.8"** (scope resolve widened past 7.5) — investigate scope/`_filter_subtopics`. (4) throughput still ~17min/2-section run (top-model orchestrator). (5) then rag-verify + `finishing-a-development-branch` → merge into `feat/component-equation-enforcement` (NOT main). **Servers were running**: backend :8766 + Vite :5175 from this worktree; backend log at `/tmp/rag-ext-dev.log`.
