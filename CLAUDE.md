...
|---|---|---|
| **Tutor finalize+verify stage** (current focus) — silent nano draft → strong finalizer streams: covers every sub-question + one box per definition. TWO routes (gpt = structured · deepseek/gemini = tolerant) converge on the same `TutorAnswer`; frontend shows a route badge. Best-effort: a failed finalize keeps the draft. Also fixed in this branch: KPSS/ADF corpus-image 404s, formal-statement dangling-cite `[N]`, formal-statement box overflow. Env `TUTOR_FINALIZE` (default OFF) · `TUTOR_FINALIZE_MODEL` (default gpt-5.4 full). | 🟢 **BUILT on worktree `feat/tutor-finalize-stage` (2026-06-18); 9 commits, backend 1098 + frontend 329 green, tsc clean.** REMAINING (Task 7): (a) fresh **frontend combined-review**; (b) **docs lockstep** — new `docs/services/chat-features/59-tutor-finalize.md`, mermaid + env row in `36-deep-tutor.md`, `tutor.html`, invariants, changelog; (c) **live browser verify on :5175** with `TUTOR_FINALIZE=1` (stationarity+versions+unit-root · KPSS/ADF · unit-root↔stationarity) — also re-certifies citation numbering + KaTeX paint; (d) final whole-branch review → `finishing-a-development-branch` → merge. **DECISION NEEDED:** flip `TUTOR_FINALIZE` default ON (every answer pays one gpt-5.4 call) vs keep opt-in — currently OFF (dormant). | [spec](docs/superpowers/specs/2026-06-17-tutor-finalize-stage-design.md) · [plan](docs/superpowers/plans/2026-06-17-tutor-finalize-stage.md) |
