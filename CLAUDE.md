...
|---|---|---|
| **Tutor finalize+verify stage** (current focus) — silent nano draft → strong finalizer streams: covers every sub-question + one box per definition. TWO routes (gpt = structured · deepseek/gemini = tolerant) converge on the same `TutorAnswer`; frontend shows a route badge. Best-effort: a failed finalize keeps the draft. Also fixed in this branch: KPSS/ADF corpus-image 404s, formal-statement dangling-cite `[N]`, formal-statement box overflow. Env `TUTOR_FINALIZE` (default OFF) · `TUTOR_FINALIZE_MODEL` (default gpt-5.4 full). | ✅ **BUILD COMPLETE + REVIEWED + LIVE-VERIFIED on worktree `feat/tutor-finalize-stage` (2026-06-18); 13 commits, backend 1101 + frontend 329 green, tsc clean.** All tasks done; docs lockstep done (doc 59, `36` mermaid+env, `tutor.html`, invariant 49, changelog). Final whole-branch review = **READY** (only doc-mermaid drift found, fixed). **Live-verified on :8767 (worktree backend, `TUTOR_FINALIZE=1`):** finalizer uses gpt-5.4 full (route=structured, `finalizeApplied`), 0 corpus-image leaks, formal-statement cite resolves, all sub-questions woven. Live caught 2 bugs (finalize defaulted to nano via `_resolve_stage_model`; `sources_full` image leak from truncate-before-rewrite) — both fixed + tested. **BROWSER PIXEL-VERIFIED on :5175 (worktree dev stack, `TUTOR_FINALIZE=1`, 2026-06-18):** badge renders `Finalized · gpt-5.4-2026-03-05 · structured`, 12 sources, formal-statement box does NOT overflow, prose clean, no image-broken icons, no console errors. **BUG FOUND IN BROWSER (pending):** the formal-statement FORMULA partially renders — `\phi_0`/`\varepsilon_t` (backslash commands) render as math but bare braced subscripts `y_{t}`, `y_{t-1}` render LITERALLY because the finalizer emits the statement with inconsistent math delimiting (those parts aren't wrapped in `$…$`). FIX: tighten the finalize prompt so `formal_statements[].statement` wraps ALL math (incl. plain-variable subscripts) in `$…$`, and/or a frontend bare-math wrap catching `letter_{…}` runs (not just `\commands`). **REMAINING:** (1) **fix the formal-statement math-delimiting bug** (above); (2) **model-switch browser test** NOT yet done — pick deepseek/gemini in the model modal → Apply → confirm `finalizeRoute` flips to `tolerant` + answer renders; (3) **DECISION** — flip `TUTOR_FINALIZE` default ON (every answer pays one gpt-5.4 finalize call; deepseek/gemini viable via Route B) vs keep opt-in — currently OFF (dormant); (4) `finishing-a-development-branch` → merge into `feat/component-equation-enforcement` + clean up the worktree. **State note:** the worktree dev stack is currently running on :5175/:8766 (the prior main-checkout dev server was stopped to run it). **Residual (OUT OF SCOPE → DR-8c):** planner may not surface "strict stationarity" as a distinct facet, so a "versions" question can still omit strict; tracked under Definition Recovery DR-8c. | [spec](docs/superpowers/specs/2026-06-17-tutor-finalize-stage-design.md) · [plan](docs/superpowers/plans/2026-06-17-tutor-finalize-stage.md) · [doc 59](docs/services/chat-features/59-tutor-finalize.md) |
