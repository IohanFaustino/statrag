...
> All agents EXCEPT you (the orchestrator) are delegated to **`ollama-cloud/...`** models through OpenCode CLI dispatch (see the "How to dispatch to Ollama agents in OpenCode" rules below in this file). Verify the provider first (`opencode providers list`, `opencode models | grep '^ollama-cloud/'`), pin the provider in the command (`opencode run --model "ollama-cloud/glm-5.1" --dangerously-skip-permissions --dir <repo> "<task>"`) — **use `ollama-cloud/glm-5.1`, never qwen** — and apply rule 0 isolation. **If dispatch cannot be forced to an `ollama-cloud/...` model, STOP and report — never silently fall back to another provider.**
>
> ### 4. 🟢 LOAD THE SUPERPOWERS SKILLS THE MOMENT YOU READ THIS FILE — before any other action.
> Reading this file means you are about to work in this repo, and rule 2 makes the superpowers skills your operating system. So **immediately, as your first tool call this session, invoke `superpowers:using-superpowers`** (the entrypoint that establishes how to find and use every other skill), and **proactively load the full suite you will need** for any non-trivial task — `brainstorming`, `writing-plans`, `using-git-worktrees`, `subagent-driven-development`, `requesting-code-review`, `receiving-code-review`, `verification-before-completion`, `finishing-a-development-branch`, `systematic-debugging`, `test-driven-development`, `dispatching-parallel-agents`, `executing-plans`, `writing-skills` — rather than discovering them mid-task. Do this on EVERY fresh session that reads this file, even if the request "looks simple": a skill check comes BEFORE clarifying questions or exploration. (Markdown cannot make the harness auto-load skills — YOU load them via the Skill tool; this rule is the standing instruction to do so unprompted.)
>
> **Red-flag thoughts that mean you are violating this contract:** *"I'll just fix this one line"* (→ dispatch), *"I'll diagnose it myself with a quick curl"* (→ consult `debug_Advisor`), *"the dirty tree won't worktree cleanly, I'll just dispatch into main with a no-git note"* (→ rule 0 violation; commit a recovery point FIRST), *"I'll let the default model implement it"* (→ ollama-cloud only), *"this is simple, I don't need the skills"* (→ rule 4: load `superpowers:using-superpowers` first, always). Stop and re-read.
...
| **Tutor multi-question facet contract (scope A)** — pure-code: `multi_question_split` + `concepts_from_asks` + `augment_concepts_and_facets` union each question's subject into concepts (→ definition recovery) + asks into facets (→ coverage + finalize); `_MAX_GAPS` 3→5. So an N-question prompt decomposes + each subject reaches the detector. | ✅ **DONE + MERGED into `feat/component-equation-enforcement` (merge `3ebec42`, 2026-06-18); branch+worktree cleaned up.** Full chat suite green (9 new tests). **Live-verified on :5175** ("What is stationarity? What are its versions? What is a unit root?"): answer decomposes into STATIONARITY / UNIT ROOT / INTEGRATED VERSIONS I(0)-vs-I(1) subsections woven by the Introduction — original "doesn't decompose multi-question prompts" complaint FIXED. **Gap (→ DR follow-up, NOT scope A):** verbatim strict/weak/unit-root formal-definition BOXES did not render — "versions" surfaced as I(0)/I(1) not strict/weak, and the Formal-statement box held only the AR(p) eqn (+ a small trailing `$$` leak). Cause: live retrieval already pulls def prose so `_has_labelled_def`→not-a-gap→no dedicated verbatim recovery (regression test used empty sources). Needs DR-5 (frontend structured render) + DR-8 (gap concepts force strict/weak/covariance even when prose present). | [spec](docs/superpowers/specs/2026-06-18-tutor-multiquestion-facet-contract-design.md) · [plan](docs/superpowers/plans/2026-06-18-tutor-multiquestion-facet-contract.md) |
