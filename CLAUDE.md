...
| **Extension mode — finish/polish + restore old chats** — new `extension` mode (deepagents topology C; corpus+Wikipedia footnote augmentation; styled-HTML ZIP export) is **BUILT + live-proven end-to-end** on branch `worktree-feat+extension-mode` (770+ backend / 207 frontend tests green; 7 live/review fixes committed; real Wikipedia footnotes w/ LaTeX rendered). **Open follow-ups:** (1) **RESTORE OLD CHATS** — relaunching `dev.sh` from the worktree pointed the SQLite store at the worktree (empty DB) → user's chats vanished from sidebar; fix store DB path / stop worktree :5175 server. (2) **Verify frontend→backend routes** (all `/api/*` incl. `/api/export` download). (3) live-confirm populated digest persists+re-renders after reload (persist-in-`finally` fix committed, untested live). (4) throughput: parallelize analysts + FS section-offload (full-chapter top-model ≈6min + 429 TPM). (5) math delimiters `\(…\)`→`$`/KaTeX + language drift (Polish leaked) + markdown `[^n]` markers. (6) subtopic scope-narrowing inconsistent. (7) browser visual verify (card + (i) modal + ZIP click). (8) rag-verify + merge into `feat/component-equation-enforcement` (NOT main). | ⏳ **NEXT — flagged 2026-06-09, built, polish+merge pending.** | [spec](docs/superpowers/specs/2026-06-09-extension-mode-design.md) · [plan](docs/superpowers/plans/2026-06-09-extension-mode.md) · [doc 54](docs/services/chat-features/54-extension-mode.md) |
