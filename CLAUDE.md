...
| **Extension v2 — story timeline + curiosity boxes (REBUILD)** — user-approved redesign replacing the deepagents extension core with a deterministic async pipeline: `scope → fetch → storyteller×N (parallel) → story_editor → subject_miner×take → researcher×subject (PURE CODE: hybrid_search rerank cross-book + Wikipedia REST) → curiosity_writer×take → citation_binder (PURE CODE: Citation fields copied VERBATIM from retrieval payloads — fixes all 4 observed citation defects) → judge (one bounded retry) → StoryDigest`. Output: timeline of story "takes" (1 section = 1 take, author's sequence, justified prose w/ KaTeX+markdown) + per-take collapsed **curiosity box** toggle (frontend layout A: numbered rail; ZIP: per-take numbered footnotes, sanitized filename). New `StoryDigestCard`; legacy `ExtensionDigest` convs keep old card (schema-keyed). Agent=Harness+model table in spec §3. 15 TDD tasks; deletes `agent.py` + `extension_skills/`. **STATUS 2026-06-10: NOT COMPLETE — built + Task 15 (live verify) IN PROGRESS, paused.** Lives ONLY in worktree `.claude/worktrees/extension-v2`, branch `feat/extension-v2-story-curiosity` (Tasks 1–14 done+reviewed; ~880 backend / 254 frontend tests green). Run from THAT worktree via `./scripts/dev.sh` (:5175). Pipeline proven end-to-end ~90s–4min (vs ~17min v1); timeline rail + justified KaTeX render; digest persists. **4 live bugs fixed:** cross-encoder meta-tensor crash (reranker main-thread warm), Wikipedia 403 (User-Agent), `sources_full` raw-meta black-screen (map to frontend `Source` + guard), wiki used verbose titles→0 hits (now mined-queries-first). **RESUME at the worktree's CLAUDE.md row** — re-run + confirm curiosity boxes populate with corpus+wiki chips, no black-screen, ZIP, reload; then opus final review + rag-verify + merge into `feat/component-equation-enforcement` (NOT main). | ⏳ **NOT COMPLETE — Task 15 live verify in progress, paused 2026-06-10.** Worktree `feat/extension-v2-story-curiosity`. | [spec](docs/superpowers/specs/2026-06-10-extension-v2-story-curiosity-design.md) · [plan](docs/superpowers/plans/2026-06-10-extension-v2-story-curiosity.md) · [doc 54](docs/services/chat-features/54-extension-mode.md) |
