...
| **Extension v2 — story timeline + curiosity boxes (REBUILD)** — deterministic async pipeline replacing deepagents core: `scope → fetch → storyteller×N → story_editor → subject_miner×take → researcher×subject (PURE CODE) → curiosity_writer×take → citation_binder (PURE CODE, verbatim payload citations) → judge (one retry) → StoryDigest`. `StoryDigestCard` (timeline rail, curiosity toggle, corpus/wiki chips). Legacy `ExtensionDigest` convs keep old card. ~880 backend / 254 frontend tests green. **NOT COMPLETE — Task 15 (live verify) IN PROGRESS, paused 2026-06-10.** Built on worktree `.claude/worktrees/extension-v2`, branch `feat/extension-v2-story-curiosity`. Tasks 1–14 done+reviewed (subagent-driven, per-task spec+quality reviews). **Live-run progress (run from extension-v2 worktree via `./scripts/dev.sh`, :5175):** pipeline runs end-to-end ~90s–4min (vs ~17min v1); timeline rail + justified prose + KaTeX render correctly; digest persists + re-renders. **4 live bugs found+fixed this session:** (1) `c29ddd4` cross-encoder meta-tensor crash in worker threads → `_warm_retrieval` now warms reranker on main thread (rerank=True); (2) `c29ddd4` Wikipedia 403 (no User-Agent) → `_WIKI_HEADERS` added; (3) `603a513` `sources_full` emitted raw evidence metas → app black-screened (ContextPanel `src.book.toLowerCase()` unguarded) → runner now maps evidence to frontend `Source` shape + SourceCard guards missing book; (4) `a1e454f` wiki lookup used verbose subject titles → 0 wiki hits → now queries mined queries first. **STILL TO VERIFY (resume here):** (a) re-run after `a1e454f`+`603a513` and confirm curiosity boxes populate with BOTH corpus AND wikipedia chips (last persisted digest f6b29fbf had 12 items/21 citations but **corpus-only** — pre-wiki-fix); (b) confirm no black-screen on completion (sources fix untested live end-to-end); (c) expand a curiosity box in browser, verify chip rendering + wiki link opens; (d) ZIP download; (e) reload persistence. **Then:** final whole-impl review (opus) + rag-verify + finishing-a-development-branch → merge into `feat/component-equation-enforcement` (NOT main). Dev log `/tmp/rag-extv2-dev.log`. | ⏳ **NOT COMPLETE — Task 15 live verify in progress, paused 2026-06-10** | [spec](docs/superpowers/specs/2026-06-10-extension-v2-story-curiosity-design.md) · [plan](docs/superpowers/plans/2026-06-10-extension-v2-story-curiosity.md) · [doc 54](docs/services/chat-features/54-extension-mode.md) |
