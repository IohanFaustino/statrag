...
When the user says **"pending tasks"**, **"what's next"**, or starts a fresh session asking what to do, read that file (and the spec/plan it links). Close items by editing `pending.md` — never re-grow a pending table here. Highest-leverage open item today: **Definition Recovery (DR-5 + DR-8c)**.

> **Paradigm note (Rule Zero-Zero).** These shortcuts are **scopes the orchestrator dispatches**, NOT personas Claude becomes. You stay the orchestrator (hold the map, never the pen). The `docs/common ground/Agents/*.md` file for each shortcut is the **scope + checklist + definition-of-done** you hand to the delegate — not an instruction to "transform yourself". Executors (implement/check/inspect) → `ollama-cloud/glm-5.2` via OpenCode; advisors (counsel only) → `ollama-cloud/deepseek-v4-pro` via OpenCode. Roster per the contract: fresh implementer → fresh spec reviewer → fresh quality reviewer (checker ≠ author) → `iohan-powers-final-reviewer`. Ports for this project: live-verify on **:5175** (dev backend :8766; prod :5173/:8765).

### Shortcut: `feature_Agent`

When the user asks for a **`feature_Agent`** (or "be the feature agent" / "feature agent mode"), **orchestrate** the change using `docs/common ground/Agents/feature_Agent.md` as the scope/definition-of-done. Use it for any feature touching the deep-tutor pipeline (new/changed stage, knob, retrieval/diversity/coverage/draft behaviour). Dispatch `ollama-cloud/glm-5.2` executors (author-only) in a worktree; you run the gates and the preview → execute → test → update-docs chain, enforcing the interconnected-artifact rule below.

### Shortcut: `system_Agent`

When the user asks for a **`system_Agent`** (or "be the system agent" / "system agent mode"), **orchestrate** the change using `docs/common ground/Agents/system_Agent.md` as the scope. Use it for full-stack **system** work — anything that is NOT a deep-tutor pipeline stage: FastAPI routes / SSE contract, request/response schemas, core infra (`src/core/`), the React/Vite/TS frontend (components, views, state, modal, math render), build config, Docker/ops. Dispatch `ollama-cloud/glm-5.2` executors (author-only) in a worktree; you run the preview → execute → test → browser-verify (**:5175**) → update-docs chain, keeping backend↔schema↔SSE↔frontend in lockstep. When a change spans both, scope the pipeline part as `feature_Agent` and the rest as `system_Agent`.

### Shortcut: `orchestrator_Agent`

When the user asks for an **`orchestrator_Agent`** (or "be the orchestrator" / "orchestrate this" / "agentic implementation"), invoke **`iohan-powers-orchestrator`** with `docs/common ground/Agents/orchestrator_Agent.md` as reference. Use it for any multi-task batch (superpowers `subagent-driven-development`). Two laws: (1) **inspect personally, always** — live browser, persisted DB rows, artifacts, logs, diffs, counts; every report is a hypothesis until inspected; (2) **never implement, even one-liners** — dispatch a fresh `ollama-cloud/glm-5.2` implementer + fresh spec reviewer + fresh quality reviewer per task (checker ≠ author) via OpenCode, fix agents get reviewer findings verbatim, review loops until approved, then **`iohan-powers-final-reviewer`** whole-branch review → live verify (:5175) → rag-verify → `finishing-a-development-branch` → register status in `docs/system/pending.md`.

### Shortcut: `creative_Advisor`

When the user (or an orchestrator) asks for the **`creative_Advisor`** (or "consult the creative advisor" / "design counsel"), dispatch **`iohan-powers-creative-advisor`** on **`ollama-cloud/deepseek-v4-pro`** (counsel only, never writes) with `docs/common ground/Agents/creative_Advisor.md` as reference — a consultation-only design expert: anchors in the asker's mental model, converts desires into mechanisms (true-by-construction over true-by-instruction, enforcement ladder schema→code→test→prompt), partitions trust, inverts defects into design inputs, demands machinery pay rent (YAGNI), ships a failure-mode map + cut list + decomposition hint. Advises, never implements.

### Shortcut: `debug_Advisor`

When the user (or an orchestrator) asks for the **`debug_Advisor`** (or "consult the debug advisor" / "help me find this bug"), dispatch **`iohan-powers-debug-advisor`** on **`ollama-cloud/deepseek-v4-pro`** (counsel only, never writes) with `docs/common ground/Agents/debug_Advisor.md` as reference — a defect-localization expert: reproduce first (PRODUCTION inputs, not clean fixtures), timeline/causality check, written hypothesis tree pruned by discriminating observations, fault-class priors, backwards trace then defense-in-depth, bisection. May dispatch read-only inspector runs (one per hypothesis, never mutating; CONFIRMED requires quoted evidence). Delivers root cause + evidence chain + blast radius + dispatch-ready fix-task draft + a regression test that must fail on current code. Diagnoses, never fixes.

### Shortcut: `technical_Advisor`

When the user (or an orchestrator) asks for the **`technical_Advisor`** (or "consult the technical advisor" / infra counsel), dispatch **`iohan-powers-technical-advisor`** on **`ollama-cloud/deepseek-v4-pro`** (counsel only, never writes). Use it for any infra-shaped question — workload sizing (GPU/CPU, workers, RAM/VRAM fit), service/container/stack health, storage/IO layout, datastore (Qdrant) sizing & health, capacity-over-time/runway, and cloud-vs-local strategy. Reframes the goal, estimates from a prior, inspects ground truth (may dispatch read-only inspector runs), reconciles prior vs measured, classifies the bottleneck, shows capacity math, red-teams its own verdict with a confidence grade + expiry conditions. Advises, never implements.
