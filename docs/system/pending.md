...
- **DR-render-leak (found via browser-verify 2026-06-28)** 🟠 — in the tutor formal-statement box, **raw inline LaTeX leaks unrendered**: e.g. "What is weak stationarity?" → the verbatim `Definition 4.2 (covariance stationarity)` box shows literal `\mathbb {Z}` (space-separated) and `\sigma` instead of ℤ / σ. Cause: the recovered verbatim def carries inline LaTeX that is NOT `$`-delimited (OCR-source style `\mathbb {Z}` with a space), so `renderFormalStatements`/the math tokenizer doesn't KaTeX-render it. Fix direction: normalize/`$`-wrap bare inline LaTeX atoms (incl. space-separated `\mathbb {Z}`, `\sigma`, greek/`\mathbb`) in the formal-statement render path (mirror the existing bare-math wrap used for the AR(1) case). NOTE: the rest of the answer rendered fine (56 KaTeX nodes); off-topic causal def confirmed GONE (DR-9/10 fix live-verified in-browser). **Delegation deferred — opencode dispatch currently hangs.** Screenshot: `tutor-weak-stationarity-formal-statement-latex-leak.png`.
